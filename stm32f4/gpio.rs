...
impl Gpio {
    ...
    pub fn enable(&self, pin: u32, config: GpioConfig) {
        unsafe {
            ...
            self.otyper
                .update_with_mask(0x1 << pin, (config.otype as u32) << pin);
            self.pupdr
                .update_with_mask(0x3 << (pin * 2), (config.pupd as u32) << (pin * 2));

            if pin < 8 {
                self.afrl
                    .update_with_mask(0xf << (pin * 4), (config.af as u32) << (pin * 4));
            } else {
                self.afrh.update_with_mask(
                    0xf << ((pin - 8) * 4),
                    (config.af as u32) << ((pin - 8) * 4),
                );
            }
            self.moder
                .update_with_mask(0x3 << (pin * 2), (config.mode as u32) << (pin * 2));
        }
    }
